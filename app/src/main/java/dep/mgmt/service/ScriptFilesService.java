...
public class ScriptFilesService {
  private static final Logger log = LoggerFactory.getLogger(ScriptFilesService.class);
  ...
  public void createTempScriptFiles() {
    ...
    List<AppDataScriptFile> scriptFiles = AppInitData.getScriptFiles();
  ...
  public boolean isScriptFilesMissingInFileSystem() {
    try {
      ...
      for (final AppDataScriptFile scriptFile : scriptFiles) {
        Path filePath = Path.of(TEMP_SCRIPTS_DIRECTORY + ConstantUtils.PATH_DELIMITER + scriptFile.getScriptFileName());
