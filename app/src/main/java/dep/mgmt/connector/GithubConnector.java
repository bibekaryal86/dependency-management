...
public class GithubConnector {
    ...
    public GithubPullRequestApi.CreateResponse createPullRequest(final String repoName, final String branchName) {
        ...
        final Map<String, String> headers = Map.of("Accept", "application/vnd.github+json",
                "Authorization", String.format("Bearer %s", CommonUtilities.getSystemEnvProperty(ConstantUtils.ENV_GITHUB_TOKEN)));
        final GithubPullRequestApi.CreateRequest requestBody = new GithubPullRequestApi.CreateRequest(ConstantUtils.GITHUB_PR_TITLE_BODY, ConstantUtils.GITHUB_PR_TITLE_BODY, branchName, ConstantUtils.GITHUB_PR_BASE_BRANCH);

        HttpResponse<GithubPullRequestApi.CreateResponse> response = Connector.sendRequest(url, Enums.HttpMethod.POST, new TypeReference<GithubPullRequestApi.CreateResponse>() {}, null, headers, requestBody);
    ...
    public GithubPullRequestApi.MergeResponse mergePullRequest(final String repoName, final Integer pullNumber) {
